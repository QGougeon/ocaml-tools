...
let create o3sA o3sB n = {
  ...
  err = (fun x _ _ _ _ ->
    print_newline();
    print_string "[ocaml-tools/memoBTable:create] - apply error: "; print_int x;
    print_newline()
  );
  each = (fun _ _ _ _ -> ());
...
let push mem a b =
  if test mem a
  then failwith "[ocaml-tools/memoBTable:push] - already stored"
...
let dump_stats mem = Tree.(Node [
    Node [Leaf "length:"; StrTree.of_int (Hashtbl.length (mem.table))];
    Node [Leaf "hit count:"; StrTree.of_int (!(mem.hitCnt))];
    Node [Leaf "clc count:"; StrTree.of_int (!(mem.clcCnt))]
  ])

let make o3sA o3sB n =
  let mem = create o3sA o3sB n in
  ( mem, (if true then apply else nocheck_apply) mem )
